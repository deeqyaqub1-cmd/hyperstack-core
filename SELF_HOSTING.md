...
# Add OPENAI_API_KEY or EMBEDDING_BASE_URL for semantic search
```
...
| `OPENAI_API_KEY` | Recommended | Enables semantic search via OpenAI embeddings (~$0.39/mo at 1K cards) |
| `EMBEDDING_BASE_URL` | Optional | Custom embedding endpoint — use instead of OpenAI (e.g. Ollama) |
| `EMBEDDING_MODEL` | Optional | Embedding model name (default: `text-embedding-3-small`) |
| `EMBEDDING_API_KEY` | Optional | API key for custom endpoint — defaults to `OPENAI_API_KEY`, then `"ollama"` |
...
| Setup | Env vars needed | Result |
|---|---|---|
| No embeddings | neither | Keyword search only — still useful |
| OpenAI | `OPENAI_API_KEY` | Full semantic + GraphRAG |
| Ollama (local) | `EMBEDDING_BASE_URL` + `EMBEDDING_MODEL` | Full semantic + GraphRAG, fully local |

---

## Fully Local Embeddings with Ollama

Run semantic search with zero external dependencies — no OpenAI account, no data leaving your network.
...
> **Note:** Use `host.docker.internal` (not `localhost`) so the Docker container can reach Ollama running on your host machine. On Linux, use `--add-host=host.docker.internal:host-gateway` in your docker run command.

**4. Start HyperStack as normal**
```bash
docker compose up -d
```

That's it — semantic search and GraphRAG work fully locally.

**Linux docker run example:**
```bash
docker run -d \
  --name hyperstack \
  -p 3000:3000 \
